...
guess_list1 = zero_area+spread*(rand([iterations,1])-1);
guess_list2 = zero_area+spread*(rand([iterations,1]));
...
% filter_list: a list of constraints used to filter the collected data
    ...
    success = false;
    [xr, success] = bisection_solver(fun, x_guess0(1), x_guess0(2))
    if ~success
        [xr, success] = secant_solver(nfun,x_guess0(1),x_guess0(2))
    end
    if ~success
        error("no root to run")
    end
    ...
    for i=1:length(guess_list1)
        ...
        switch solver
            case "bisection"
                bisection_solver(fun, guess_list1(i), guess_list2(i));
            case "newton"
